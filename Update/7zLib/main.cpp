...
void PrintError(std::string sz)
{
  std::cerr<<std::endl<<"ERROR: "<<sz<<std::endl;
}
...
size_t Decompress7zArchive(std::string const& archivePath, std::string rootPath)
{
    ...
    //utiliser    "  x: eXtract files with full paths\n");
    ...
    if (res == SZ_OK)
    {
        ...
        //if (strcmp(command, "x") == 0) { fullPaths = 1; }
        ...
        if (res == SZ_OK)
        {
            ...
            */
            ...
            for (i = 0; i < db.NumFiles; i++)
            {
                ...
                std::vector<char16_t> name(len+1);

                SzArEx_GetFileNameUtf16(&db, i, (uint16_t*)&name[0]);
                ...
                */
                ...
                std::cout<<"Extracting ";
                std::for_each(name.begin(),name.end(), for_each_cout<char>);
                ...
                {
                    std::vector<char16_t>::iterator it=std::find(name.begin(), name.end(), '/');
                    if(it!=name.end())
                    {
                    ...
                    name.insert(name.begin(), rootPath.begin(), rootPath.end());
                    if(name[0]=='/')
                    {
                ...
                for (size_t j = 0; j<(name.size()-1); j++)
                {
                    ...
                    {
                        name[j] = '\0';
                        MyCreateDir((const UInt16*)&name[0]);//(const UInt16*)
                ...
                if (isDir)
                {
                    MyCreateDir((const UInt16*)&name[0]);
                    std::cout<<std::endl;
                    continue;
                }

                else if (OutFile_OpenUtf16(&outFile, (UInt16*)&name[0]))
                {
                ...
                SetFileAttributesW((LPCWSTR)&name[0], db.Attribs.Vals[i]);
                #endif
                printf("\n");
            }
...
{
    return Decompress7zArchive(archivePath, "");
}

#ifdef MY_DEBUG
int MY_CDECL main(int numargs, char *args[])
{
    Decompress7zArchive("T\\Exeptions.7z", "T\\U");
    return 0;
}
#endif // MY_DEBUG
